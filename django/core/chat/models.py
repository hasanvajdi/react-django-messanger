...
def profile_avatar(instance, filename):
    return f'profile_avatar/{instance.user.id}_avatar'
...
def ChannelAvatr(instance):
    return f'channel_avatr/{instance.group_id}_avatar'

def ChannelIdCreator():
    return int(''.join(random.sample("0123456789", 14)))
...
class ChannelStructure(models.Model):
    channel_id  = models.CharField(max_length = 100, blank=True, null=True, default=ChannelIdCreator)
    link        = models.CharField(max_length=100, blank=True, null=True, default=ChannelLinkCreator)
    name        = models.CharField(max_length=50, blank=False, null=False)
    avatar      = models.FileField(upload_to = ChannelAvatr)
    biograhpy   = models.CharField(max_length = 400, blank = True, null = True)
